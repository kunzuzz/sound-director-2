                ...
                <div className="tracks-list">
                  {(() => {
                    ...
                    return sortedTracks.map((track, index) => {
                      const trackStatus = getTrackChangeStatus(scene.name, track);
                      const audioId = `${scene.name}-${track.name}`;
                      const waveformData = generateWaveformData(audioId);
                      
                      // Check if this is a scene with multiple tracks that need a selection dropdown
                      const sceneOptions = getMultipleTrackScenes();
                      const hasMultipleTracks = sceneOptions[scene.name];
                      
                      // For scenes with multiple tracks, only show the selected track in display mode
                      const shouldShowTrack = mode === 'edit' || 
                                             !isMultipleTrackScene || 
                                             (isMultipleTrackScene && 
                                              track.name === (selectedOptions[scene.name] || 
                                                             (hasMultipleTracks ? hasMultipleTracks.mainTrack.name : track.name)));
                      
                      // Only render the track item if it should be visible
                      if (!shouldShowTrack) {
                        return null;
                      }
                      ...
                      return (
                        <div 
                          ...
                          className={`track-item ${
                            trackStatus === 'removed' ? 'track-removed' : 
                            trackStatus === 'added' ? 'track-added' :
                            trackStatus === 'changed' ? 'track-changed' : ''
                          }`}
                          draggable={mode === 'edit'}
                          onDragStart={(e) => handleDragStart(e, track, scene.name)}
                        >
                          {track.isTag && (
                            <span className="tag-label">{track.tagName}</span>
                          )}
                          
                          {/* For scenes with multiple tracks, make the "select" tag clickable to reorder tracks */}
                          {isMultipleTrackScene && track.isTag && track.tagName === 'select' && mode === 'edit' && (
                            <button 
                              className="select-btn"
                              onClick={() => reorderTracks(scene.name, track.tagName, track.name)}
                            >
                              Select
                            </button>
                          )}
                          
                          {/* For scenes with multiple tracks, just show "selected" as a tag */}
                          {isMultipleTrackScene && track.isTag && track.tagName === 'selected' && (
                            <span className="selected-tag">{track.tagName}</span>
                          )}
